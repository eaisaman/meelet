    ...
    function () {
        return function (appModule, extension, meta) {
            ...
            appService.prototype.getChatConfiguration = function () {
                ...
                return self.$http({
                    method: 'GET',
                    url: (window.serverUrl || "") + '/api/private/chatConfiguration',
                    params: {}
                }).then(function (result) {
                    if (result.data.result === "OK") {
                        var chatOptions = result.data.resultValue;
                        return self.utilService.getResolveDefer({data: {result: "OK", resultValue: chatOptions}});
                    } else {
                        return self.utilService.getRejectDefer(result.data.reason);
                    }
                }, function (err) {
            ...
            appService.prototype.getServerUrl = function () {
                return this.utilService.getResolveDefer({
                    data: {
                        result: "OK",
                        resultValue: location.protocol + "//" + location.hostname + ":" + (location.port || "80")
                    }
                });
            };
            ...
            appService.prototype.loadPomelo = function () {
                ...
                if (window.pomelo) {
                    ...
                    return self.$q.all([
                        self.getChatConfiguration(),
                        self.getDeviceId()
                    ]).then(function (result) {
                        var chatOptions = result[0].data.result == "OK" && result[0].data.resultValue || {};
                        var deviceId = result[1].data.result == "OK" && result[1].data.resultValue || "";

                        chatOptions.deviceId = deviceId;
                        chatOptions.chatRoute = chatOptions.chatRoute || self.angularConstants.pomeloRoute.defaultChatRoute;
                        (window.pomeloContext = window.pomeloContext || {}).options = chatOptions;
                        if (window.pomeloContext.load) {
                            return self.utilService.timeout(function () {
                                    var defer = self.$q.defer();

                                    window.pomeloContext.load(chatOptions, function () {
                                        defer.resolve(window.pomelo);
                                    });
            ...
            appService.prototype.initPomelo = function () {
                var self = this;
                ...
                if (window.pomeloContext) {
                    ...
                    return self.utilService.timeout(
                        function () {
                            var defer = self.$q.defer(),
                                newInstance = new window.pomelo();

                            newInstance.init({
                                host: window.pomeloContext.options.host,
                                port: window.pomeloContext.options.port,
                                deviceId: window.pomeloContext.options.deviceId,
                                reconnect: true
                            }, function () {
                                window.pomeloContext.pomeloInstance = newInstance;
                                defer.resolve();
                            }, function (err) {
                                defer.reject(err);
                            });
            ...
            appService.prototype.getChat = function (userId, chatId) {
                var self = this;
            ...
            appService.prototype.acceptChatInvitation = function (chatId, userId, deviceId, route) {
                ...
                return self.$http({
                    method: 'PUT',
                    url: (window.serverUrl || "") + '/api/private/acceptChatInvitation',
                    params: {
                        userId: userId,
                        chatId: chatId,
                        deviceId: deviceId,
