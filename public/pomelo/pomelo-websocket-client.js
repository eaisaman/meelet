...
(function (exports, GLOBAL) {
    exports.WebSocket = exports.WebSocket || require('ws');
}('object' === typeof module ? module.exports : this, this));
...
(function (exports, GLOBAL) {
    ...
    pomelo.prototype.connect = function () {
        ...
        self.socket = new exports.WebSocket(self.url);
        ...
        self.socket.onopen = function () {
            ...
            if (self.socket) {
                try {
                    self.socket.send(Package.encode(Package.TYPE_HANDSHAKE, Protocol.strencode(JSON.stringify(self.handshakeBuffer))));
                    ...
                    console.log('websocket send handshake data failed: %j', err);
                    self.disconnect();
    ...
    pomelo.prototype.send = function (reqId, route, msg) {
        ...
        try {
            self.socket.send(Package.encode(Package.TYPE_DATA, msg));
        } catch (err) {
            console.log('websocket send binary data failed: %j', err);
    ...
    pomelo.prototype.handshake = function (data) {
        ...
        try {
            self.socket.send(Package.encode(Package.TYPE_HANDSHAKE_ACK));
            ...
            console.log('websocket send handshake ack data failed: %j', err);
    ...
    pomelo.prototype.heartbeat = function () {
        ...
        self.heartbeatId = setTimeout(function () {
            ...
            if (self.socket) {
                try {
                    self.socket.send(Package.encode(Package.TYPE_HEARTBEAT));
                } catch (err) {
                    console.log('websocket send heartbeat data failed: %j', err);
                    self.disconnect();
                    self.doReconnect(err);
                }
            }
